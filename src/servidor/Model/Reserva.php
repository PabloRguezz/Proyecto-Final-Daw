...
public function insert_reserva($hora_reserva, $nombre_servicio, $id_servicio, $id_usuario) {
  $conectar = parent::connection();
  parent::set_name();
  $sql = "INSERT INTO Reserva(hora_reserva, nombre_servicio, id_servicio, id_usuario) VALUES (?, ?, ?, ?)";
  $stmt = $conectar->prepare($sql);
  $stmt->bindValue(1, $hora_reserva, PDO::PARAM_STR);
...
public function update_reserva($id_reserva, $hora_reserva, $nombre_servicio, $id_servicio, $id_usuario) {
  $conectar = parent::connection();
  parent::set_name();
  $sql = "UPDATE Reserva SET hora_reserva=?, nombre_servicio=?, id_servicio=?, id_usuario=? WHERE id_Reserva=?";
  $stmt = $conectar->prepare($sql);
  $stmt->bindValue(1, $hora_reserva, PDO::PARAM_STR);
  $stmt->bindValue(2, $nombre_servicio, PDO::PARAM_STR);
  $stmt->bindValue(3, $id_servicio, PDO::PARAM_INT);
